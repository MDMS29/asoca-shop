...
class ProductosModel extends Model
{
    ...
    protected $allowedFields = ['nombre', 'descripcion', 'categoria', 'cantidad_actual', 'cantidad_vendida', 'precio', 'fecha_public', 'estado', 'fecha_crea', 'usuario_crea']; /* relacion de campos de la tabla */
    ...
    {
        $this->select('tbl_productos.id_producto, tbl_productos.nombre, descripcion, categoria, tbl_param_det.nombre as nomCate, cantidad_actual, cantidad_vendida, precio, fecha_public, concat(tbl_usuarios.nombre_p, " ", tbl_usuarios.apellido_p) as nomCreador, tbl_img_producto.nombre_img');
        $this->join('tbl_usuarios', 'tbl_usuarios.id_usuario = tbl_productos.usuario_crea');
        $this->join('tbl_param_det', 'tbl_param_det.id_param_det = tbl_productos.categoria');
        $this->join('tbl_img_producto', 'tbl_img_producto.id_producto = tbl_productos.id_producto');
        if ($nombre != '') {
    ...
    {
        $this->select('tbl_productos.id_producto, tbl_productos.categoria as id_categoria,tbl_param_det.nombre as categoria, tbl_productos.nombre, precio, tbl_img_producto.nombre_img, CAST(AVG(tbl_valoracion_producto.valoracion) AS INT) as valoracion');
        $this->join('tbl_img_producto', 'tbl_img_producto.id_producto = tbl_productos.id_producto');
        $this->join('tbl_param_det', 'tbl_param_det.id_param_det = tbl_productos.categoria');
        $this->join('tbl_valoracion_producto', 'tbl_valoracion_producto.id_producto = tbl_productos.id_producto', 'left');
        ...
        switch (gettype($categoria)) {
            case 'string':
                $this->where('tbl_param_det.nombre', $categoria);
                break;
            default:
